...
def handle_keys():
    ...
    if objeto_atual == 1:
        ...
        if keys[K_a]: cube_pos[0] -= 0.1
        if keys[K_d]: cube_pos[0] += 0.1
        #teclas do triangulo(Y,G,H,J)
        if keys[K_y]: tri_pos[1] += 0.1
        if keys[K_h]: tri_pos[1] -= 0.1
        if keys[K_g]: tri_pos[0] -= 0.1
        if keys[K_j]: tri_pos[0] += 0.1

    elif objeto_atual == 4:
        #teclas da piramide(W,A,S,D)
